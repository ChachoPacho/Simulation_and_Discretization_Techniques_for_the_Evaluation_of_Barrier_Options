...
\subsection{Calibración de la Geometría (Malla Gruesa)}
...
\subsubsection{Restricción Espacial: Alineación con la Barrera} 
...
\subsubsection{Restricción Temporal: El Parámetro de Estiramiento} 
...
\subsubsection{Cálculo de Probabilidades de la Malla Base} 

Con $h$ y $k$ calibrados, se calculan las probabilidades de transición para los nodos de la malla gruesa. A diferencia de los árboles centrados en la media, aquí el árbol está anclado en la barrera, por lo que incluimos el término \textit{drift} en las probabilidades.
...
\subsection{Gestión de la Interfaz entre Mallas}
...
\subsubsection{Cálculo de Probabilidades Ajustadas (Ecuación de Transición)} 
...
\subsubsection{Valoración de los Nodos Frontera} 
...
\begin{equation} 
  V(B_{2,j}) = e^{-r (k/4)} \left[ p_u(\Delta t_{eff}) V(A_u) + p_m(\Delta t_{eff}) V(A_m) + p_d(\Delta t_{eff}) V(A_d) \right] \label{eq:amm_interface_val} 
\end{equation}

\textbf{Nota crítica de implementación:} Observe que el factor de descuento es siempre $e^{-r(k/4)}$, porque estamos retrocediendo solo un micro-paso en el árbol fino, aunque las probabilidades se calculen proyectando hasta el final del intervalo grueso.

\subsubsection{Relleno de la Malla Fina} 
...
\subsection{Valoración Backward y Recursividad}
...
\subsubsection{Fase 1: Resolución de la Malla Gruesa (Conjunto $A$)} 
...
\subsubsection{Fase 2: Inyección de Valores en la Frontera Fina} 
...
\begin{itemize} 
  \item Para los nodos $B$ que coinciden con tiempos $A$ ($t,t+k,\dots$), se copia el valor directamente o se usa interpolación espacial si los precios no alinean perfectamente (aunque la calibración de la Ec. \ref{eq:amm_h_calib} busca evitar esto). 
...
\subsubsection{Fase 3: Resolución de la Malla Fina (Conjunto $B$)} 
...
\subsubsection{Generalización Recursiva (Para $M>1$)} 
...
\begin{verbatim}
def Resolver_AMM(S0, K, T, r, sigma, H, M):
  ...
  for level in range(1, M + 1):
    fine_grid = build_fine_mesh(current_grid, current_h, current_k, current_N, r, 
                  sigma, H, K) # La nueva malla fina se "injerta" en la actual
...
def build_coarse_mesh(S0, K, T, r, sigma, H, h, k, N):
  pu_A, pm_A, pd_A = calcular_probs(k, h, r, sigma) # Eq. 9 para malla gruesa
...
\subsection{Estructuras de Datos y Memoria}
...
\subsubsection{Arquitectura de Almacenamiento Disperso} 
...
\subsubsection{Complejidad Computacional y Escalabilidad} 
...
\subsubsection{1. Calibración Geométrica y Parámetro de Estiramiento} 
...
\subsubsection{2. Probabilidades de Transición en la Interfaz} 
...
\begin{lstlisting}[language=Python, caption={Cálculo dinámico de probabilidades para nodos de interfaz}] 
  def calcular_probs(dt_eff, dx, r, sigma): 
    """ 
...
\subsubsection{3. El Bucle de Refinamiento Recursivo} 
...
\subsection{Metodología de Validación (Benchmark)} 
...
\subsection{Prueba de Estrés: El Problema del Límite} 
