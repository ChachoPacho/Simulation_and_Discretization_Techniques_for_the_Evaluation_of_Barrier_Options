...
Este bucle es la implementación práctica de la propiedad de isomorfismo del AMM. Como el modelo es isomorfo, se pueden añadir niveles sucesivos de refinamiento aplicando exactamente el mismo procedimiento matemático. En el código, esto se refleja en cómo el bucle toma la malla fina recién generada y la convierte en la ``malla gruesa'' de la siguiente iteración (\texttt{current\_grid = fine\_grid}).

\begin{lstlisting}[language=Python, caption={Generalización recursiva y bucle de refinamiento}]
def Resolver_AMM(S0, K, T, r, q, sigma, H, M):
  # 1. CALIBRACION GLOBAL (Para la malla mas profunda)
  h_coarse_base, k_base, N_base = calibrar_geometria(
                                    S0, H, T, sigma, M)

  # 2. CONSTRUIR MALLA BASE (NIVEL 0)
  A_grid = build_coarse_mesh(S0, K, T, r, q, sigma, H, 
                          h_coarse_base, k_base, N_base)
  ...
  for level in range(1, M + 1):
    ...
    fine_grid = build_fine_mesh(current_grid, current_h,
                                current_k, current_N, r,
                                q, sigma, H, K)
    
    current_grid = fine_grid # La fina de hoy es la gruesa de manana
...
def build_coarse_mesh(S0, K, T, r, q, sigma, H, h, k, N):
  # Eq. 9 para malla gruesa
  pu_A, pm_A, pd_A = calcular_probs(k, h, r, q, sigma)
...
def build_fine_mesh(coarse_grid, h_coarse, k_coarse, 
  N_coarse, r, q, sigma, H, K):   
  total_fine_steps = N_coarse * 4
  B_grid = initialize_fine_mesh()
  
  # 1. INYECCION DE NODOS (Eq. 11)
  for j in range(N_coarse): # Logica de interpolacion (Eq. 12 y 13)
    t_start = j * 4

    # El source siempre es la malla previa
    B_grid[2, t_start] = coarse_grid[1, j]
    
    # Nodos intermedios (Interpolacion)
    V_Ad, V_Am, V_Au = coarse_grid[:, j+1]
    
    for sub_step in range(1, 4):
      # Distancia efectiva a proximo A
...
Esta estructura garantiza que la información fluya correctamente desde la resolución más baja hacia la más alta (flujo \textit{downward} para barreras), permitiendo obtener el precio de alta precisión en $t=0$.
...
\begin{table}[h]
\centering
\caption{Comparación de convergencia cerca de la barrera ($S_0=90.125$).}
\label{tab:amm_results}
  \begin{tabular}{|l|c|c|c|}
    \hline
    \textbf{Método} & \textbf{Timesteps} & \textbf{Valor Calculado} & \textbf{Err. Relativo} \\
    & \textbf{(N)} & & \\
    \hline Solución Exacta & - & \textbf{$\approx 0.161648$} & - \\
    (Merton) & & & \\
    \hline Árbol Trinomial & N/A & \textit{Fallo (Oscilación)} & $>100\%$ \\
    ($M=0$) & & & \\
    \hline AMM Nivel 2 & $6083$ & $0.161312$ & $0.21\%$ \\
    ($M=2$) & & & \\
    \hline AMM Nivel 4 & $380$ & \textbf{$0.161532$} & $0.07\%$ \\
    ($M=4$) & & & \\
    \hline
  \end{tabular}
\end{table}
