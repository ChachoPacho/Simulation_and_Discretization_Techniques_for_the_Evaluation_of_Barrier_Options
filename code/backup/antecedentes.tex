...
\begin{itemize}
    \item \textbf{Knock-in}: La opción vale si y sólo si se alcanzó la barrera.
    \begin{itemize}
...
\section{Modelo de Black-Scholes para Opciones Barrera}
\label{section:bs_barreras}

\subsection{Fundamentos del Modelo Black-Scholes}
El modelo de Black-Scholes \cite{black1973pricing} constituye la base teórica fundamental para la valoración de opciones europeas. Para opciones barrera, este modelo permite obtener fórmulas cerradas exactas bajo ciertas condiciones de regularidad.

\subsubsection{Supuestos del Modelo}
El modelo de Black-Scholes se basa en los siguientes supuestos fundamentales:
\begin{itemize}
    \item El precio del subyacente $S_t$ sigue un Movimiento Browniano Geométrico
    \item Es posible entrar en posición short en cualquier instrumento
    \item No hay costos de transacción
    \item Los activos son divisibles en cualquier cantidad
    \item Los activos no tienen dividendos
    \item No hay posibilidad de arbitraje
    \item Las transacciones se realizan de manera continua en el tiempo
    \item La tasa de interés libre de riesgo, $r$, es constante y es la misma para todos los plazos
\end{itemize}

\subsubsection{Ecuación Diferencial de Black-Scholes}
Bajo estos supuestos, el precio de cualquier derivado $V(S(t))$ que depende del precio del subyacente $S$ y del tiempo $t$ debe satisfacer la ecuación diferencial parcial de Black-Scholes:

\begin{equation}
\frac{\partial V}{\partial t} + \frac{1}{2}\sigma^2 S^2 \frac{\partial^2 V}{\partial S^2} + rS\frac{\partial V}{\partial S} - rV = 0
\end{equation}

Para opciones barrera, esta ecuación debe resolverse sujeta a condiciones de borde específicas que incorporan el comportamiento en la barrera.

\subsection{Solución Analítica de Reiner y Rubinstein}
\label{subsec:reiner_rubinstein}

El trabajo seminal de Reiner y Rubinstein \cite{reiner1991breaking} proporciona una metodología sistemática para obtener fórmulas cerradas para los ocho tipos básicos de opciones barrera europeas.

\subsubsection{Marco Metodológico de Valoración}
...
\textbf{Interpretación de los Términos de Valoración}
...
\subsubsection{Fórmulas Exactas: Construcción mediante Términos de Valoración}
