...
El problema es que en Monte Carlo avanzamos hacia adelante en el tiempo, pero la decisión de ejercicio requiere conocer el valor de continuación, que depende del futuro. Esto crea una paradoja circular: para decidir si ejercer en el momento $t_j$, necesitamos calcular el valor esperado de continuación $\mathbb{E}[V(S_{t_{j+1}}, t_{j+1}) | S_{t_j}]$. Sin embargo, el valor de la opción en el siguiente paso, $V(S_{t_{j+1}}, t_{j+1})$, depende inherentemente de las decisiones de ejercicio futuras, las cuales a su vez dependen de valores de continuación en períodos posteriores. Por lo tanto, esta interdependencia hace que la decisión óptima no pueda abordarse directamente mediante una simulación estándar hacia adelante, requiriendo un enfoque metodológico más sofisticado.

\section{Método de Longstaff-Schwartz (LSM)}

La simulación de Monte Carlo para opciones americanas enfrenta el desafío de la direccionalidad del tiempo. Como vimos en la sección anterior, la decisión de ejercer anticipadamente requiere comparar el valor intrínseco inmediato con el valor de continuación esperado, el cual, a su vez, depende de decisiones de ejercicio en períodos subsiguientes, generando una dependencia circular que es imposible de resolver de manera secuencial hacia el futuro.

El método propuesto por \cite{longstaff2001valuing}, conocido como \textit{Least Squares Monte Carlo} (LSM), resuelve este problema mediante un enfoque estadístico basado en la regresión de mínimos cuadrados. En lugar de avanzar hacia adelante para evaluar la estrategia óptima, el algoritmo genera primero todas las trayectorias de precios completas hasta el vencimiento y luego retrocede en el tiempo (\textit{backward induction}).

En cada punto de decisión $t_j$, el modelo estima el valor de continuación condicional aproximándolo como una función del precio actual del activo mediante regresión transversal (es decir, evaluando simultáneamente el corte transversal de todas las trayectorias simuladas en ese instante exacto, utilizando típicamente polinomios de Laguerre como funciones base). Para las trayectorias que están \textit{in-the-money}, se efectúa una regresión del valor descontado realizado en $t_{j+1}$ sobre el precio actual $S_{t_j}$. Esto permite comparar empíricamente el beneficio de ejercer la opción inmediatamente frente al de mantenerla viva, convirtiendo un complejo problema de optimización dinámica en una secuencia manejable de problemas de regresión.
